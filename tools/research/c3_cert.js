/**
 * C3 인증 (전좌석 마스터) — 주공이 '위 서열이 밖에 남은' 기루다 국면에서
 * 비기루다 점수카드를 리드하기로 한 결정을, 같은 정책의 차선
...
const SEED0 = parseInt(process.env.SEED_BASE || "2000000", 10);
...
function fireCond(g, seat, act) {
  ...
  for (let r = c.rank + 1; r <= 14; r++) if (!seen.has(c.suit + r)) return true;  // 그 무늬 위 서열 잔존
  return false;
}
async function nextBest(sess, g, seat) {
  ...
  for (let i = 0; i < M.ACTION_DIM; i++) {
    ...
    if (i >= A_PLAY0 && i < A_PLAY0 + 52) {
      ...
      // 클래스 액션(위 서열 잔존 오프수트 점수 리드) 자체는 후보에서 제외
      if (cd.suit !== gi && E.isPointCard(cd) && !E.sameCard(cd, g.mightyCard)) {
        let higher = false;
        for (let r = cd.rank + 1; r <= 14; r++) if (!seen.has(cd.suit + r)) { higher = true; break; }
        if (higher) continue;
      }
    }
