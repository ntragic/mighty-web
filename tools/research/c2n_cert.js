/**
 * C2n 인증 (전좌석 마스터) — C2의 좁힌 재인증: 공개 전, '무늬 가시 탑'인
 * 점수카드(그 무늬 위 서열이 전부 출현·내 손)를 이기지 못하는 트릭에 태우는
 * 결정만 교정. C2 전체(잡점수 포함)는 −126±89 유의 손해로 기각 —
 * 컨트롤 카드(탑) 헌납만 좁히면 다른가 (2026-08-08 제보: 주공 ♣A 헌납 −283).
 * 사용: node tools/research/c2n_cert.js [판수]   env MODEL·SEED_BASE
 */
'use strict';
...
const SEED0 = parseInt(process.env.SEED_BASE || '2300000', 10);
...
function topOfSuit(g, seat, c) {
  ...
  for (let r = c.rank + 1; r <= 14; r++) if (!seen.has(c.suit + r)) return false;
  return true;
}
function fireCond(g, seat, act) {
  if (g.friendRevealed || act.jokerCall) return false;   // 공개 전 (주공 포함 — 제보 케이스가 주공)
  const pl = g.play;
  if (!pl.table.length) return false;
  const c = act.card;
  if (E.isJoker(c) || E.sameCard(c, g.mightyCard) || !E.isPointCard(c)) return false;
  if (!topOfSuit(g, seat, c)) return false;               // 무늬 가시 탑만
...
async function nextBest(sess, g, seat) {
  ...
  for (let i = 0; i < M.ACTION_DIM; i++) {
    ...
    if (i >= A_PLAY0 && i < A_PLAY0 + 52) {
      ...
      if (E.isPointCard(cd) && !E.sameCard(cd, g.mightyCard) && topOfSuit(g, seat, cd)
          && !gt(g._cardStrength({ player: seat, card: cd }, g.play), bk)) continue; // 지는 탑 점수카드 제외
    }
