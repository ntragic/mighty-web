...
function fireCond(g, seat, act) {
  ...
  // 여당 확정 좌석만: 주공, 공개 프렌드, 또는 프렌드 카드 보유자
  const fd = g.friendDecl;
  const iAmFriend = (g.friendRevealed && g.friend === seat) ||
    (fd && fd.mode === 'card' && fd.card && g.hands[seat].some(x => E.sameCard(x, fd.card)));
  if (seat !== g.declarer && !iAmFriend) return false;
  const c = act.card;
