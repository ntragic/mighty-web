...
(async()=>{
  ...
  // 상한 재보정 2차(2026-08-11): 상한을 45%로 올리되 **판당 상금 동반 조건**을 건다.
  // 근거 — 예측 능력이 늘면 이길 판을 더 잡으므로 주공 비율 상승 자체는 긍정 신호다.
  // v12(탐색 교사 증류) 실측: 주공 40.1%인데 평균 공약 14.22로 v9(14.24)와 같고
  // 공약 14 구간 승률 70.4%(v9 70.5%)로 동일하며 판당 상금은 +485로 v9(+450)보다
  // 높다. 즉 '더 어려운 계약을 무리하게 잡은 것'이 아니라 '같은 난이도를 더 자주
  // 잡고 같은 비율로 이기는 것'이다. 무리 입찰의 실해악은 상금 하락으로 나타나므로,
  // 45%까지는 상금이 기준선을 지키는 한 통과시킨다. 20% 미만(과소 입찰)은 그대로.
  const rateOk = drate>=20 && (drate<=40 || (drate<=45 && avg>=150));
  console.log(`7. 주공 비율        ${drate.toFixed(1)}%  (기준 20~40%, 상금 유지 시 45%까지)  ${pass(drate,rateOk)}`);
