...
const settle = num('FINAL_TRICK_SETTLE');
ok(hold !== null && hold >= 800, `FINAL_TRICK_HOLD가 없거나 너무 짧다: ${hold}`);
ok(settle !== null && settle >= 300, `FINAL_TRICK_SETTLE이 없거나 너무 짧다: ${settle}`);
...
const iCollect = src.indexOf('// 수거 애니메이션');
...
// 4) 세대 카운터 무효화가 홀드 뒤에도 걸려 있어야 한다(되돌리기·새 라운드 경합)
...
console.log(`timing: ${pass} passed, ${fail} failed ` +
  `(마지막 트릭 ${hold}ms + 정산 전 ${settle}ms · 세팅 show ${(cs && (cs[1].match(/show:\s*(\d+)/) || [])[1]) || '?'}ms)`);
