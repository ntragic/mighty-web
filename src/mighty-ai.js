...
function keyCardGuard(game, seat, action, trace) {
  ...
  try {
    ...
    // 기루다가 리드되면 합법수가 전부 기루다라 위 필터가 모두 걸러진다. 예전에는
    // 여기서 정책 수를 그대로 통과시켰고, 그래서 야당 ♠A로 이미 잠긴 트릭에 주공이
    // ♠5를 두고 ♠K를 버리는 장면이 남았다(제보 seed 2135136798 6트릭 — 그 1점으로
    // 공약 15가 깨졌다). 못 이기는 트릭에 한해 '덜 비싼 카드'로 내려 준다.
    // 이기는 수(iWin)는 건드리지 않는다 — 인증도 그 조건으로 쟀다.
    // 야당 방향의 같은 함정은 tfeedGuard가 먼저 고쳤다(dfeed→tfeed 재정의) — 이건 그 여당판이다.
    // 인증(tools/research/kfollow_cert.js, v16e 전좌석 마스터):
    //   여당 발화 판 주공 상금 +96.3±78.2 (n=216, 8,943딜) · 여당 승수 +4 · 발화 0.025회/판
    //   야당 −10.9±36.6 중립 (n=658, 5,576딜) · 발화 0.135회/판
    if (iWin) { T('no-cheap-alt'); return action; }
    const cheap = game._legalPlays(seat).filter(m =>
      !m.jokerCall && !isKey(m.card) && !beats(m.card, m.jokerSuit));
    if (!cheap.length) { T('no-cheap-alt'); return action; }
    cheap.sort((a, b) => cost(a) - cost(b));
    if (cost(cheap[0]) >= cost({ card: c })) { T('no-cheap-alt'); return action; }
    T('downgraded');
    return { type: 'play', card: cheap[0].card };
