...
const I18N_EN = {
  ...
  '조율가':'Balancer', '기억가':'Recaller', '수련생':'Apprentice',
...
function planFor(tier){ return TIER_PLAN[tier]||TIER_PLAN.intermediate; }
  ...
  box.innerHTML=`<h2>${t('매치 종료')}</h2><div class="sub">${tf('matchSub', why, roundNo)}</div>
    ...
    ${order.map((p,i)=>`<div class="rank-row${i===0?' first':''}">
      <div class="no">${i+1}</div><div class="nm">${NAMES[p]}${p===HUMAN?t('(나)'):
        (nnSeatsActive()&&poolOf(seatModels[p])?` <span class="style-tag">${t(poolOf(seatModels[p]).nick)}</span>`:'')}</div>
