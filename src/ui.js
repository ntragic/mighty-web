...
const APP_BUILD = '2026-08-14 빌드 — 혼합 풀 조정';
const HUMAN = 0;
...
/* 혼합 운영 — 좌석마다 다른 세대를 앉혀 성향차를 낸다.
 * 플레이 중 비노출(비딩 읽힘 방지 — 페르소나와 같은 원칙), 매치 종료 화면에서
 * 사후 공개. 코칭·AI 복기 판정은 항상 대표(MASTER_MODEL=v13).
 * v2.10: 좌석 배분을 추첨에서 **고정 배분 후 섞기**로 바꿨다 — 추첨이면 한
 * 세대가 네 좌석을 모두 차지하는 판이 생겨 성향차라는 취지가 흐려진다.
 * v2.10.6: 책사(v9)를 풀에서 내렸다. 고공약(여유 0) 판에서 프렌드가 조커를
 * 아끼는 결함이 제보로 드러났고(seed 386134368 트릭3), 같은 국면에서 v13·v8은
 * 조커를 1순위로 내는데 v9만 3순위로 밀었다. 그 자리는 v13이 받는다 — v8을
 * 늘리면 성향은 다양해지나 v8이 v9/v13보다 판당 86 약해(9,000판 h2h) 기력이
 * 내려간다. 상세는 docs/MODELS.md. */
const MASTER_POOL=[
  { id:'v13', file:'./model/mighty_master_v13.onnx', nick:'선견가' },
  { id:'v8', file:'./model/mighty_master_v8.onnx', nick:'수문장' },
];
const SEAT_PLAN=['v13','v13','v13','v8'];    // AI 좌석 4개에 배분할 모델
